...
//	BlurWeights[11] = 0.008764;
	...
	{    
		...
		float2 tap2=center.xy + TexelKernelh[i].xy / d;		
		float3 tap2_color=tex2D(s_image,tap2).rgb * BlurWeights[i];
		
		sum += tap_color + tap2_color;
...
//	o[1]=float2(-0.840144f,-0.073580f)*scale;
	o[1]=float2(-0.695914f,0.457137f)*scale;
//	o[3]=float2(-0.203345f,0.620716f)*scale;
	o[2]=float2(0.962340f,-0.194983f)*scale;
//	o[5]=float2(0.473434f,-0.480026f)*scale;
	o[3]=float2(0.519456f,0.767022f)*scale;
//	o[7]=float2(0.185461f,-0.893124f)*scale;
	o[4]=float2(0.507431f,0.064425f)*scale;
//	o[9]=float2(0.896420f,0.412458f)*scale;
	o[5]=float2(-0.321940f,-0.932615f)*scale;
